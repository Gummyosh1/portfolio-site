...
fetch("riftbound/database/Database.csv")
...
loadBulkButton.addEventListener("click", function () {
  ...
  if (trainerActive && !pokemonInit){
    fetch("riftbound/database/pokemonDatabase.csv")
...
async function searchCard(cardName) {
  const res = await fetch(
    `https://YOUR-VERCEL-PROJECT.vercel.app/api/riftbound-card?name=${encodeURIComponent(cardName)}`
  );
